...
class SpdrMV(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        date = str(datetime.date.today())
        time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
        body = '{"searchTasks":{"RESULT_LIST":{"start":1,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":25,"size":27},"FAST_ACCESS":{},"SEARCH_WORD_HITS":{}},"filters":{"CATEGORY":["Gesetze"]},"searches":[],"clientID":"bsmv","clientVersion":"bsmv - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}' % (date, time)
    ...
    def parse_scrolldown(self, response):
        ...
        if "resultList" in results:
            ...
            date = str(datetime.date.today())
            time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
            body = '{"searchTasks":{"RESULT_LIST":{"start":%s,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":%s,"size":27},"FAST_ACCESS":{}},"filters":{"CATEGORY":["Gesetze"]},"searches":[],"clientID":"bshe","clientVersion":"bshe - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}' % (batch, batch + 25, date, time)
            batch += 25
        ...
        if "resultList" in results:
            for result in results["resultList"]:
                ...
                r = {
                    ...
                    "court": None,
                    "date": result["date"],
                    "link": "https://www.landesrecht-mv.de/bsmv/document/" + quote(result["docId"]) + '/part/X',
                    "docId": result["docId"],
