...
class SpdrBE(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        date = str(datetime.date.today())
        time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
        body = '{"searchTasks":{"CATEGORY_HITS":{},"RESULT_LIST":{"start":1,"size":51,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":52,"size":50},"SEARCH_WORD_HITS":{}},"filters":{"CATEGORY":["Gesetze"]},"searches":[],"clientID":"bsbe","clientVersion":"bsbe - V06_07_00 - 23.06.2022 11:20","r3ID":"s%sT%sZ"}' % (date, time)
    ...
    def parse_scrolldown(self, response):
        ...
        if "resultList" in results:
            ...
            date = str(datetime.date.today())
            time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
            body = '{"searchTasks":{"RESULT_LIST":{"start":%s,"size":50,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":%s,"size":50},"FAST_ACCESS":{}},"filters":{"CATEGORY":["Gesetze"]},"searches":[],"clientID":"bsbe","clientVersion":"bsbe - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}' % (batch, batch + 50, date, time)
        ...
        if "resultList" in results:
            for result in results["resultList"]:
                ...
                if len(result['titleList'])>2:
                    ...
                    #result["docId"] = quote(result["docId"], safe=':/')
                    r = {
                        "postprocess": self.postprocess,
                        "wait": self.wait,
                        "court": result["titleList"][0],
                        "date": result["date"],
                        "az": None,
                        "link": "https://gesetze.berlin.de/bsbe/document/" + result["docId"] + "/part/X",
                        "docId": result["docId"],
                        "xcsrft": self.headers["x-csrf-token"]
                    }
                if self.filter:
