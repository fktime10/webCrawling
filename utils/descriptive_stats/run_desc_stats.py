...
def compute_doc_level_metrics(row: pd.Series, config:dict, column:str):
    ...
    if config['num_unique_words']:
        ...
        row['num_unique_words'] = len(list(set([token.text for token in doc if token.is_alpha])))
...
def generate_distributions(config: dict, res: pd.DataFrame):
    metrics = [metric for metric in config['metrics'] if config['metrics'][metric] and metric!="distributions"]
    for metric in metrics:
        ...
        plt.figure()
    convert_images_to_pdf([f'{config["output_dir"]}/counts.png'] + [f"{config['output_dir']}/res_{i}.png" for i in metrics])
    # shutil.rmtree("")

def persist_metric_counts(src_name:str, df:pd.DataFrame, counts: pd.DataFrame, metrics: list):
    for metric in metrics:
        counts.loc[src_name, metric] = df[metric].mean() if 'ratio' in metric else df[metric].sum()
    return counts
...
def compute_metrics(config: dict):
    ...
    for src in sources:
        ...
        df = pd.read_json(S3_PATH.format(bucket, src), storage_options=aws_creds, lines=True)
        print("File read complete")
        src_metrics_df = df.swifter.apply(compute_doc_level_metrics, config=config['metrics'], column=column, axis=1)
    ...
    counts = counts.style.set_table_attributes("style='display:inline'").set_caption('Total counts per sources.')
